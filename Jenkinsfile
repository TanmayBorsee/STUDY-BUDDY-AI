pipeline {
    agent any
    // environment {
    //     DOCKER_HUB_REPO = "gaya891/studybuddy"
    ...
    stages {
        stage('Checkout Github') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'github-token', url: 'https://github.com/TanmayBorsee/STUDY-BUDDY-AI.git']])
            }
        }        
        ...
        //                 sh '''
        //                 argocd login 34.134.235.90:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
